...
@Service
public class IDFCBankServiceImpl implements MultipartyReconServiceInterface {
	...
	public void readTransactionFile(File file, String username) throws Exception {
		...
		CSVReader reader;
	        ...
	        line=reader.readNext();
	        String[] headerLine = {"Transaction Date", "Payment date", "Narrative", "Customer Reference No", "Cheque No", "Debit", "Credit", "Running Balance"};
	        ...
	        while((line=reader.readNext())!=null)
	        {
	        	...
	        	if(lineArr[5].equalsIgnoreCase("0.00"))
	        	{	
	        		bankTransactionEntity.setTransactionAmount(Double.parseDouble(lineArr[6].replaceAll(",", "")));
	        		bankTransactionEntity.setCreditFlag("C");
	        	} else 
	        	{
	        		bankTransactionEntity.setTransactionAmount(Double.parseDouble(lineArr[5].replaceAll(",", "")));
	        		bankTransactionEntity.setCreditFlag("D");
	        	}
	        	
	        	bankTransactionEntity.setBalanceAfterTxn(Double.parseDouble(lineArr[7].replaceAll(",", "")));
	        	bankTransactionEntity.setBankCode("IDFC Bank");
	        	bankTransactionEntity.setRemarks(lineArr[2]);
	        	bankTransactionEntity.setFileName(fileName);
	        	bankTransactionEntity.setPartner("IDFC");
	        	bankTransactionEntity.setTxnType("ONLINE_DEPOSIT");
	        	try {
